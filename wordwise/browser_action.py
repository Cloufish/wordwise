...
def _run_bulk_fetch(col, note_ids, cfg) -> BulkSummary:
    ...
    for nid in note_ids:
        ...
        result = fetcher.fetch_for_note(
            col, note, profile, cfg["pexels_api_key"], overwrite=cfg.get("overwrite_existing", False)
        )
        if result.skipped_no_word:
            summary.skipped_no_word += 1
            continue

        if result.frequency_set or result.image_set or result.gender_set:
...
def _on_success(browser: Browser, summary: BulkSummary, overwrite: bool) -> None:
    browser.table.reset()
    mode = "overwrite existing values" if overwrite else "skip existing values"
    text = (
        "Wordwise fetch complete.\n\n"
        f"Mode: {mode}\n"
...
def _fetch_selected(browser: Browser) -> None:
    ...
    if not cfg["pexels_api_key"]:
        showWarning(
            "No Pexels API key configured. Frequency will still be fetched, "
            "but images will be skipped until you add a key in Tools ▸ Wordwise Settings…."
        )
    ...
    QueryOp(
        ...
        success=lambda summary: _on_success(browser, summary, overwrite),
